param(
    [int]$Timesteps = 1000,
    [string]$OutputRoot = "results/dormitory/sl8000_zone_capacity_screen"
)
...
$output = Join-Path $outputDir "sl8000_zone-capacity_t${Timesteps}.json"
$args = @(
    ...
    "--matcherScalableZoneCapacity", "true", "--matcherScalableZoneCapacityLimit", "8",
    "--matcherScalableZoneCapacityWeight", "20", "--matcherScalableZoneCapacityRadius", "1"
)

& $exe @args 2>&1 | Tee-Object -FilePath ([System.IO.Path]::ChangeExtension($output, ".log"))
if ($LASTEXITCODE -ne 0) { throw "SL-8000 zone-capacity matcher failed with exit code $LASTEXITCODE" }

$result = Get-Content -LiteralPath $output -Raw | ConvertFrom-Json
[pscustomobject]@{
    Method = "taskmatcher-zone-capacity"
