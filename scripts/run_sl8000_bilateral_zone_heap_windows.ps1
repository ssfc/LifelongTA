param(
    [int]$Timesteps = 1000,
    [string]$OutputRoot = "results/dormitory/sl8000_bilateral_zone_heap"
)
...
$output = Join-Path $outputDir "sl8000_bilateral-zone-heap_t${Timesteps}.json"
$args = @(
    ...
    "--matcherScalableGlobalFraction", "0.25", "--matcherScalableBilateralHeap", "true",
    "--matcherScalableTaskCandidates", "8", "--matcherScalableZoneRadius", "4"
)

& $exe @args 2>&1 | Tee-Object -FilePath ([System.IO.Path]::ChangeExtension($output, ".log"))
if ($LASTEXITCODE -ne 0) { throw "SL-8000 bilateral zone heap failed with exit code $LASTEXITCODE" }

$result = Get-Content -LiteralPath $output -Raw | ConvertFrom-Json
[pscustomobject]@{
    Method = "taskmatcher-bilateral-zone-heap"
