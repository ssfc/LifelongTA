...
$input = Join-Path $repo ("instances/warehouseSmall/warehouseSmall_{0}.json" -f $Agents)
if (!(Test-Path $exe)) { throw "Missing executable: $exe" }
if (!(Test-Path $input)) { throw "Missing instance: $input" }
if (!$OutputDir) { $OutputDir = "results/lab/warehouse-small_{0}_strict_compare" -f $Agents }
...
foreach ($method in $methods) {
    $output = Join-Path $fullOutputDir ("warehouse-small_{0}_{1}_{2}.json" -f $Agents, $method.Name, $Timesteps)
...
$rows = foreach ($method in $methods) {
    $path = Join-Path $fullOutputDir ("warehouse-small_{0}_{1}_{2}.json" -f $Agents, $method.Name, $Timesteps)
