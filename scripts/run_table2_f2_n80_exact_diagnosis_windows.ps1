param(
    ...
    [string]$OutputDir = "results/dormitory/table2_f2_n80_exact_diagnosis"
)
...
function Invoke-Case([string]$Method, [int]$Seed) {
    ...
    if ($Method -eq 'flow-unit') {
        # Exact options of scripts/run_table2_flow_unit.sh.
        $args += @('--scheduleModel', '1', '--useTraffic', 'false')
    } elseif ($Method -eq 'taskmatcher-free') {
        # Exact options of scripts/run_table2_taskmatcher_free_expand.sh.
...
Write-Host "Exact f=2,n=80 diagnostic stage complete: $out"
