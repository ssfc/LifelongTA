...
OUT_ROOT="$ROOT/results/table2_wait_priority_screen"
TIMESTEPS=${1:-400}
  ...
  for n in 50 80 100; do
    for seed in 0 1 2; do
      ...
      "$BIN" --inputFile "$INPUT_ROOT/problem_${n}_${f}_${seed}.json" --output "$output" --outputScreen 3 \
        ...
        --matcherMaxAssign 0.5 --matcherAdaptiveAssign true --matcherWaitPriorityWeight 1.0 \
        --matcherWaitPriorityThreshold 20 --matcherReassign true --heapKeepBias 0 \
        --heapProtectDist 0 --matcherUseTraffic false >"$log" 2>&1
    done
  done
done
