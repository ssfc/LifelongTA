...
# Screen read-only planner edge-flow reranking on the local SL16 Heap setup.
ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
BIN="$ROOT/build/lifelong"
OUT_DIR="$ROOT/results/table1_sl16_traffic_rerank_screen"
TIMESTEPS=${1:-250}
...
run_one() {
  ...
  printf '[%s] %s: traffic-rerank-weight=%s\n' "$(date '+%F %T')" "$label" "$weight"
  "$BIN" --inputFile "$ROOT/instances/sortationLarge/sortationLarge_16000.json" \
    ...
    --heapTrafficRerankTopK 4 --heapTrafficRerankSteps 4 \
    --heapTrafficRerankWeight "$weight" --logDetailLevel 2 >"$log" 2>&1
}

run_one traffic0 0.0
run_one traffic025 0.25
run_one traffic100 1.0
run_one traffic400 4.0

printf '\n\a========== SL16 traffic-rerank screen complete ==========\n'
date '+Completed at %F %T %Z'
wall 'SL16 traffic-rerank screen has completed.' 2>/dev/null || true
