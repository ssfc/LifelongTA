...
OUT_ROOT="$ROOT/results/table2_guide_regret_screen"
TIMESTEPS=${1:-400}

[[ -x "$BIN" ]] || { echo "Missing executable: $BIN" >&2; exit 1; }
mkdir -p "$OUT_ROOT"

# The Planner-Aware screen was behind Flow-Unit in these four cells. The
# guide-regret term is screened alone: its long-tail companion regressed the
# f=2,n=50 smoke test and is intentionally excluded here.
for spec in "2 100" "5 50" "5 100" "10 50"; do
  read -r f n <<<"$spec"
  for seed in 0 1 2; do
    ...
    "$BIN" --inputFile "$INPUT_ROOT/problem_${n}_${f}_${seed}.json" --output "$output" --outputScreen 3 \
      ...
      --matcherGuideRegretWeight 0.5 --matcherGuideRegretCap 20 \
      --heapKeepBias 0 --heapProtectDist 0 >"$log" 2>&1
  done
done

printf '\n\a========== Guide-regret TaskMatcher Table 2 screen complete ==========\n'
