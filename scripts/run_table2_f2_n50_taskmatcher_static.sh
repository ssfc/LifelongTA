...
OUT_DIR="$ROOT/results/table2_f2_n50_taskmatcher_static"
TIMESTEPS=${1:-400}
...
run_one() {
  ...
  output="$OUT_DIR/taskmatcher_static_${seed}.json"
  log="${output%.json}.log"
  ...
  "$BIN" --inputFile "$INPUT_ROOT/problem_50_2_${seed}.json" --output "$output" --outputScreen 3 \
    ...
    --matcherMaxAssign 1 --matcherReassign true --heapKeepBias 0 --heapProtectDist 0 \
