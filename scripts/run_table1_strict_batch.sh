...
for family_and_sizes in \
    ...
    for team_size in $sizes; do
        for method in greedy flow-unit flow-traffic flow-avg-waiting; do
            output="results/table1_strict/${family}_${team_size}_${method}_1000.json"
            [[ -f "$output" ]] && continue
            scripts/run_table1_strict.sh "$family" "$team_size" "$method" 1000
        done
    done
done
