...
OUT_DIR="$ROOT/results/portable_greedy_heap/no_reassign_dev"
TIMESTEPS=${1:-1000}
...
run_one() {
  ...
  output="$OUT_DIR/${family}_${team_size}_w10_no_reassign_t${TIMESTEPS}.json"
  log="${output%.json}.log"
  ...
  "$BIN" \
    ...
    --heapReassign false --heapKeepBias 6 --heapProtectDist 10 \
