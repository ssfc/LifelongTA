param(
    [int[]]$Seeds = @(5, 6, 7, 9, 13, 15, 16, 17, 21, 23),
    [string]$SourceRoot = "C:\Users\34288\.codex\worktrees\table2-diagnose\LifelongTA",
    [string]$OutputDir = "results/dormitory/table2_f2_n80_contraflow_loss_focus"
)
...
function Invoke-Case([string]$Method, [int]$Seed) {
    $stem = "f2_n80_${Method}_seed$Seed"
...
# Alternate methods inside each fixed task stream to limit machine-state drift.
foreach ($seed in $Seeds) {
    Invoke-Case 'flow-unit' $seed
    Invoke-Case 'taskmatcher-cw050' $seed
}
...
Write-Host "Counter-flow loss-focus comparison complete: $out"
