...
OUT_DIR="$ROOT/results/portable_greedy_heap/no_lns_dev"
TIMESTEPS=${1:-1000}
...
run_one() {
  ...
  output="$OUT_DIR/${family}_${team_size}_w10_lns0_t${TIMESTEPS}.json"
  log="${output%.json}.log"
  ...
  "$BIN" \
    ...
    --heapRebuildPct 45 --heapLnsPct 0 --heapSortK 500 \
