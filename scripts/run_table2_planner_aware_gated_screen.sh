...
OUT_ROOT="$ROOT/results/table2_planner_aware_gated_screen"
TIMESTEPS=${1:-400}
  ...
  for n in 50 80 100; do
    for seed in 0 1 2; do
      ...
      "$BIN" --inputFile "$INPUT_ROOT/problem_${n}_${f}_${seed}.json" --output "$output" --outputScreen 3 \
        ...
        --matcherUseTraffic true --matcherTrafficPressure 1.0 --matcherTrafficTopK 100 \
        --matcherTrafficCongestionWeight 1.0 --matcherUseWaitHeat true --matcherWaitHeatWeight 0.5 \
        --matcherWaitHeatPressure 1.0 --heapKeepBias 0 --heapProtectDist 0 >"$log" 2>&1
    done
  done
done
