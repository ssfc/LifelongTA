...
# Screen the number of traffic-rescored candidate pickups on W500. The traffic
# penalty stays at the previously selected value of 1.0.
ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
BIN=${BIN:-"$ROOT/build/lifelong"}
OUT_DIR="$ROOT/results/portable_assignment/traffic_topk_w500_screen"
TIMESTEPS=${1:-100}

[[ -x "$BIN" ]] || { echo "Missing executable: $BIN" >&2; exit 1; }
mkdir -p "$OUT_DIR"

for top_k in 25 50 75 100; do
  output="$OUT_DIR/w500_k${top_k}_cw1p0_t${TIMESTEPS}.json"
  log="${output%.json}.log"
  if [[ -s "$output" ]] && [[ $(jq -r '.numTaskFinished // 0' "$output") -gt 0 ]]; then
    continue
  fi
  echo "Running W500 traffic top-k=$top_k"
  "$BIN" --inputFile "$ROOT/instances/warehouseSmall/warehouseSmall_500.json" \
...
printf '\n\a========== W500 traffic top-k screen complete ==========\n'
