param(
    [int]$Timesteps = 1000,
    [string]$OutputRoot = "results/dormitory/sl8000_taskmatcher_future_flow"
)
...
$output = Join-Path $outputDir "sl8000_taskmatcher-future-flow_t${Timesteps}.json"
$args = @(
    ...
    "--matcherFutureFlow", "true", "--matcherFutureFlowZoneRows", "20",
    "--matcherFutureFlowZoneCols", "10", "--matcherFutureFlowWeight", "1.0"
)

& $exe @args 2>&1 | Tee-Object -FilePath ([System.IO.Path]::ChangeExtension($output, ".log"))
if ($LASTEXITCODE -ne 0) { throw "SL-8000 TaskMatcher future-flow failed with exit code $LASTEXITCODE" }

$result = Get-Content -LiteralPath $output -Raw | ConvertFrom-Json
[pscustomobject]@{
    Method = "taskmatcher-future-flow"
