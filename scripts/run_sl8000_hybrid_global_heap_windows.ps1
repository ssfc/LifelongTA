param(
    [int]$Timesteps = 1000,
    [string]$OutputRoot = "results/dormitory/sl8000_hybrid_global_heap"
)
...
function Invoke-Case([double]$Fraction) {
    $tag = [int]($Fraction * 100)
    $output = Join-Path $outputDir "sl8000_hybrid-global-heap-${tag}_t${Timesteps}.json"
    $args = @(
        ...
        "--matcherScalableGlobalFraction", "$Fraction"
    )
    & $exe @args 2>&1 | Tee-Object -FilePath ([System.IO.Path]::ChangeExtension($output, ".log"))
    if ($LASTEXITCODE -ne 0) { throw "Hybrid fraction $Fraction failed with exit code $LASTEXITCODE" }
}

Invoke-Case 0.25
Invoke-Case 0.50

Get-ChildItem -LiteralPath $outputDir -Filter "sl8000_hybrid-global-heap-*_t${Timesteps}.json" |
    Where-Object { $_.Name -notlike "*.metrics_summary.json" } | Sort-Object Name | ForEach-Object {
