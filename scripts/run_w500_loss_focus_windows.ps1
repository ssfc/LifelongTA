param(
    [int]$Timesteps = 1000,
    [int]$Trials = 3,
    [string]$OutputDir = "results/dormitory/w500_loss_focus_flow_vs_taskmatcher"
)
...
function Invoke-Case([string]$Method, [int]$Trial) {
    $label = "w500_{0}_t{1}_trial{2}" -f $Method, $Timesteps, $Trial
...
# Alternate methods to reduce time-dependent machine effects.
for ($trial = 1; $trial -le $Trials; ++$trial) {
    Invoke-Case "flow-traffic" $trial
    Invoke-Case "taskmatcher-k50" $trial
}

$summary = Get-ChildItem -LiteralPath $fullOutputDir -Filter "w500_*_t${Timesteps}_trial*.json" |
    Where-Object { $_.Name -notlike '*.metrics_summary.json' } |
    Sort-Object Name |
    ForEach-Object {
        $result = Get-Content -LiteralPath $_.FullName -Raw | ConvertFrom-Json
        $metricsPath = $_.FullName -replace '\.json$', '.metrics_summary.json'
        ...
        [pscustomobject]@{
            ...
            ActiveTasksAtEnd = [int]$metrics.activeTasksAtEnd
...
$summary | Export-Csv -NoTypeInformation -Encoding utf8 -Path (Join-Path $fullOutputDir "comparison_summary.csv")
$summary | Format-Table -AutoSize | Out-String | Set-Content -Encoding utf8 (Join-Path $fullOutputDir "comparison_summary.txt")
