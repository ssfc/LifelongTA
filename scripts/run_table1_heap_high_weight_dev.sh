...
OUT_DIR="$ROOT/results/portable_greedy_heap/high_weight_dev"
TIMESTEPS=${1:-1000}
...
run_one() {
  local family=$1 team_size=$2 weight=$3 input output log
  case "$family" in
  ...
  output="$OUT_DIR/${family}_${team_size}_w${weight}_t${TIMESTEPS}.json"
  log="${output%.json}.log"
  ...
  "$BIN" \
    ...
    --heapDistWeight "$weight" --heapMaxAssign 1.0 \
...
for spec in \
  'warehouse-small 600 15' \
  'warehouse-small 600 20' \
  'random 1200 15' \
  'random 1200 20' \
  'random 2000 15' \
  'random 2000 20'; do
  read -r family team_size weight <<<"$spec"
  run_one "$family" "$team_size" "$weight"
done
