param(
    [int]$Timesteps = 250,
    [string]$OutputDir = 'results/dormitory/ws500_ws600_service_route_screen'
)
...
foreach ($warehouse in @(500, 600)) {
    ...
    foreach ($weight in @('0.05', '0.15')) {
        $stem = "w${warehouse}_route-w${weight}_t${Timesteps}"
        ...
        $arguments = @(
            ...
            '--matcherTrafficServiceRouteWeight', $weight,
