...
# Screen the scheduler-only projected-load refinement.  Every run uses the
# released f=2, n=80 finite workload and the same one-second planner budget.
ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
BIN="$ROOT/build/lifelong"
OUT_ROOT="$ROOT/results/table2_f2_n80_projected_load_screen"

[[ -x "$BIN" ]] || { echo "Missing executable: $BIN" >&2; exit 1; }

run_one() {
  local method=$1 seed=$2 output log weight
  ...
  if [[ "$method" == static ]]; then
    ...
    weight=${method#projected_}
  fi
  extra=()
  if [[ "$method" != static ]]; then
    extra=(--matcherUseProjectedLoad true --matcherProjectedTopK 100 \
           --matcherProjectedLoadWeight "$weight" --matcherProjectedIterations 2)
  fi
  "$BIN" --inputFile "$ROOT/instances/problem_80_2_${seed}.json" --output "$output" \
    --outputScreen 3 --simulationTime 400 --planTimeLimit 1000 --preprocessTimeLimit 30000 \
...
for seed in $(seq 0 4); do
  for method in flow_unit static projected_0.10 projected_0.25 projected_0.50; do
    run_one "$method" "$seed"
  done
done

printf '\a========== Table 2 projected-load screen complete ==========\n'
date '+Completed at %F %T %Z'
printf '\aTable 2 projected-load TaskMatcher screening results are ready.\n' | wall 2>/dev/null || true
