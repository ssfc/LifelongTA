                               ...
                               SharedEnvironment* env, bool new_only, float dist_weight)
{
    ...
    // The framework's schedule vector is the authoritative assignment state.
    // Task::agent_assigned can be one synchronization step behind it.
    vector<int> task_owner(env->task_pool.size(), -1);
    int max_task_id = -1;
    for (const auto& entry : env->task_pool)
    {
        max_task_id = std::max(max_task_id, entry.first);
    }
    task_owner.assign(max_task_id + 1, -1);
    ...
    for (const auto& entry : env->task_pool)
    {
        ...
        else if (new_only)
        {
            ...
            {
                // Assignment throttling keeps unopened tasks with their owner;
                // only genuinely unassigned work enters the heap this round.
                proposed_schedule[owner] = entry.first;
            }
        }
        else
        {
