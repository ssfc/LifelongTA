...
{
    if (env == nullptr || location < 0 || location >= static_cast<int>(env->assignment_flow.size()) || direction < 0 || direction >= 4) return -1;
...
void record_assignment_planner_context(TaskManager::TaskMetric& metric, SharedEnvironment* env,
                                       int agent_location, int pickup)
{
    // Static shortest-path first-hop proxy, not the movement selected by the planner.
    if (env == nullptr || agent_location < 0 || pickup < 0 || agent_location >= static_cast<int>(DefaultPlanner::global_neighbors.size())) return;
    metric.pickup_local_degree = pickup < static_cast<int>(DefaultPlanner::global_neighbors.size()) ? static_cast<int>(DefaultPlanner::global_neighbors[pickup].size()) : -1;
...
// Record accepted assignments without changing task execution state.
void TaskManager::record_assignment_metrics(const vector<int>& assignment,
                                            const vector<State>* states,
                                            int assignment_timestep)
{
    ...
    for (const auto& next : next_agent)
    {
        ...
        {
            metric.first_assigned_at = assignment_timestep;
            metric.first_agent = next.second;
            if (metric_env != nullptr && states != nullptr &&
                next.second < static_cast<int>(states->size()) && !metric.locations.empty())
                metric.pickup_distance_at_assignment = DefaultPlanner::get_h(
                    metric_env, states->at(next.second).location, metric.locations.front());
            if (metric_env != nullptr && states != nullptr &&
                next.second < static_cast<int>(states->size()) && !metric.locations.empty())
                record_assignment_planner_context(metric, metric_env,
                    states->at(next.second).location, metric.locations.front());
        }
        else if (metric.last_assigned_agent >= 0 && metric.last_assigned_agent != next.second)
        {
            ++metric.reassignments;
        }
        metric.current_agent = next.second;
        metric.last_assigned_agent = next.second;
    }
...
// Validate before recording metrics or changing the current assignments.
                                      ...
                                      int assignment_timestep)
{
    ...
    record_assignment_metrics(assignment, states,
                              assignment_timestep < 0 ? curr_timestep : assignment_timestep);
    ...
    for (int k = 0; k < num_of_agents; k++)
    {
        if (current_assignment[k] != -1 && states[k].location == ongoing_tasks[current_assignment[k]]->get_next_loc())
        {
            ...
            if (task->is_finished())
            {
                ...
                if (task->agent_assigned >= 0 &&
                    task->agent_assigned < static_cast<int>(agent_metrics.size()))
...
void TaskManager::sync_shared_env(SharedEnvironment* env) 
{
    ...
    env->task_source_exhausted = task_id >= tasks.size();
}
...
void TaskManager::reveal_tasks(int timestep)
{
    ...
    for (int i = 0; i < num_tasks_reveal; i++)
    {
        ...
        TaskMetric metric;
        metric.task_id = task->task_id;
        metric.revealed_at = timestep;
        metric.locations = task->locations;
        task_metrics.emplace(task->task_id, std::move(metric));
...
void TaskManager::update_tasks(vector<State>& states, vector<int>& assignment, int timestep,
                               const vector<State>* before_states)
{
    curr_timestep = timestep;
    // Keep the original assignment/pickup/completion order. Only telemetry uses
    // the pre-move snapshot to include the first step of a new assignment.
    const int assignment_timestep = before_states != nullptr && !before_states->empty()
        ? before_states->front().timestep : timestep;
    set_task_assignment(assignment, before_states == nullptr ? &states : before_states,
                        assignment_timestep);
    if (before_states != nullptr) record_agent_step(*before_states, states);
...
{
    const int count = std::min({num_of_agents, static_cast<int>(before.size()),
    ...
    for (int agent = 0; agent < count; ++agent)
    {
        ...
        if (task_id < 0)
        {
            ++agent_metrics[agent].idle_steps;
            continue;
        }
        ...
        TaskMetric* task_metric = nullptr;
        const auto metric_it = task_metrics.find(task_id);
        if (metric_it != task_metrics.end()) task_metric = &metric_it->second;
...
void TaskManager::save_metrics(const std::string& result_file) const
{
    ...
    std::vector<const TaskMetric*> tasks;
    tasks.reserve(task_metrics.size());
    for (const auto& entry : task_metrics) tasks.push_back(&entry.second);
    std::sort(tasks.begin(), tasks.end(), [](const TaskMetric* lhs, const TaskMetric* rhs) {
    ...
    for (const TaskMetric* task_ptr : tasks)
    {
    ...
    for (const TimelineMetric& metric : timeline_metrics)
    {
        ...
        planner_seconds.push_back(metric.planner_seconds);
    }
    ...
    summary["assignmentMetricTiming"] = "before first movement under accepted assignment";
    summary["firstHopMetricType"] = "static shortest-path proxy, not executed planner movement";
    summary["reassignmentDefinition"] = "agent changes, including across unassigned intervals";
    summary["timeoutStepAccounting"] = "agent-state steps only; task progress remains paused";
    ...
    summary["agentStateSteps"] = {{"idle", idle_steps}, {"empty", empty_steps}, {"loaded", loaded_steps},
                                  {"activeWait", active_wait_steps}};
