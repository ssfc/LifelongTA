...
void TaskScheduler::plan(int time_limit, std::vector<int> & proposed_schedule)
{
    ...
    {
        DefaultPlanner::PortableTailBottleneckStats tail_stats;
        DefaultPlanner::schedule_plan_portable_task_matcher(limit, proposed_schedule, env,
            task_matcher_config, background_flow, task_matcher_config.tail_bottleneck ? &tail_stats : nullptr);
        if (tail_stats.eligible) {
            env->tail_bottleneck_decisions.push_back({
                {"timestep", env->curr_timestep}, {"source_exhausted", env->task_source_exhausted},
                {"eligible", tail_stats.eligible}, {"attempted", tail_stats.attempted},
                {"accepted", tail_stats.accepted}, {"deadline_fallback", tail_stats.deadline_fallback},
                {"infeasible", tail_stats.infeasible}, {"invalid_input", tail_stats.invalid_input},
                {"baseline_complete", tail_stats.baseline_complete},
                {"task_count", tail_stats.task_count}, {"agent_count", tail_stats.agent_count},
                {"baseline_tail", tail_stats.baseline_tail}, {"candidate_tail", tail_stats.candidate_tail},
                {"locked_tail", tail_stats.locked_tail},
                {"baseline_flexible_tail", tail_stats.baseline_flexible_tail},
                {"candidate_flexible_tail", tail_stats.candidate_flexible_tail},
                {"baseline_pickup", tail_stats.baseline_pickup}, {"candidate_pickup", tail_stats.candidate_pickup},
                {"elapsed_us", tail_stats.elapsed_us}
            });
        }
    }
    else if (solver == 8)
