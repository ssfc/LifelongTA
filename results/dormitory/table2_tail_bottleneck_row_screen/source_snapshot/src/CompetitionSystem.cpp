...
void BaseSystem::simulate(int simulation_time)
{
    ...
    for (; simulator.get_curr_timestep() < simulation_time; )
    {
        ...
        for (int i = 0 ; i< timeout_timesteps; i ++){
            ...
            const vector<State> timeout_states = simulator.move(all_wait_actions);
            task_manager.record_agent_step(previous_states, timeout_states);
        ...
        // Preserve pre-move states for assignment and movement telemetry.
        const vector<State> previous_states = simulator.get_current_state();
...
void BaseSystem::saveResults(const string &fileName, int screen) const
{
    ...
    // Preserve the historical field, which counts active steps per agent.
    js["maxAgentActiveSteps"] = makespan;
    const int simulation_steps = simulator.get_curr_timestep();
    const size_t total_tasks = task_manager.get_total_task_count();
    const bool all_tasks_completed = task_manager.num_of_task_finish == total_tasks;
    js["taskSourceType"] = "finite";
    js["numTasksTotal"] = total_tasks;
    js["simulationSteps"] = simulation_steps;
    js["allTasksCompleted"] = all_tasks_completed;
    js["actualMakespan"] = all_tasks_completed
        ? json(total_tasks == 0 ? 0 : simulation_steps) : json(nullptr);
    js["tailBottleneckDecisions"] = env->tail_bottleneck_decisions;
