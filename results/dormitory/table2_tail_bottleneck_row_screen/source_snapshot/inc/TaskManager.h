...
public:
    ...
    void update_tasks(vector<State>& states, vector<int>& assignment, int timestep,
                      const vector<State>* before_states = nullptr);
...
private:
    void record_assignment_metrics(const vector<int>& assignment, const vector<State>* states,
