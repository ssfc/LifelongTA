...
bool TaskManager::set_task_assignment(vector<int>& assignment, const vector<State>* states)
{
    ...
    // Capture assignment changes before replacing current_assignment.  A task
    // may be temporarily released and picked up by another agent before its
    // first pickup; those changes are exactly the reassignments we want to
    // expose in the experiment data.
...
void TaskManager::save_metrics(const std::string& result_file) const
{
    ...
    long long total_empty_distance = 0;
    long long total_loaded_distance = 0;
    long long total_service_distance = 0;
    ...
    summary["movement"] = {
        {"emptyDistance", total_empty_distance},
        {"loadedDistance", total_loaded_distance},
